...
export default async function ServicesPage({
  ...
  // --- Fetch initial data ---
  const [initialServicesData, serviceTypeData, serviceCityData] =
    await Promise.all([
  ...
  const initialServices = Array.isArray(initialServicesData)
    ? initialServicesData
    : [];
  const allServiceTypes = Array.isArray(serviceTypeData) ? serviceTypeData : [];
  const allServiceCities = Array.isArray(serviceCityData)
    ? serviceCityData
    : [];
