...
export default function checkServiceValidityOnStep(service: IServiceClientState, currentStep: number): boolean {    
    const serviceBasicInfoValid = !!service.name && !!service.description && !!service.type;
    ...
    const serviceCategoryValid = service.category.length > 0;
