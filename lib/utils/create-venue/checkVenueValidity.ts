...
export default function checkVenueValidityOnStep(venue: IVenueClientState, currentStep: number): boolean {    
    const venueBasicInfoValid = !!venue.name && !!venue.description && !!venue.type && !!venue.seating?.seated && !!venue.seating.standing;
    ...
    const venueCategoryValid = venue.category.length > 0;
