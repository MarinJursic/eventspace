...
const VenueDetailClient: React.FC<VenueDetailClientProps> = ({
  ...
  const handleDateTimeSelection = useCallback(
    (selectedDates: string[], timeSlot: string) => {
      ...
      toast({
        title: "Dates Updated",
        description: `${sortedDates.length} day(s) selected for ${initialVenue.name}`,
      });
  ...
  const handleBookVenue = useCallback(() => {
    if (!initialVenue) {
      toast({
        title: "Error",
        description: "Venue data missing.",
        variant: "destructive",
      });
      return;
    }
    ...
    const sortedDates = [...selectedVenueDates].sort(
      (a, b) => new Date(a).getTime() - new Date(b).getTime()
    );
  ...
  const handleExternalVenueConfirm = useCallback(
        ...
        <main className="flex-grow pt-8 pb-16 bg-gradient-to-b from-white via-secondary/10 to-white">
          <div className="container mx-auto px-4 flex flex-col items-center justify-center min-h-[60vh] text-center">
            <p className="text-lg text-muted-foreground mb-4">
              Venue details could not be loaded.
            </p>
