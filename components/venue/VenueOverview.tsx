...
import { Venue } from "@/lib/mockVenues"; // Import Venue type to use its nested types
...
const VenueOverview: React.FC<VenueOverviewProps> = ({
  ...
  return (
    <div className="space-y-3"> {/* Added spacing */}
      <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight">{name}</h1>
      ...
      {(city || address) && (
        <div className="flex items-center text-sm text-muted-foreground mt-1">
          ...
          <span>{city ? `${city}` : ''}{city && address ? ', ' : ''}{address || ''}</span>
        </div>
      )}
