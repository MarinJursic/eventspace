...
interface VenueGalleryProps {
  images: { // This structure matches Venue['images']
    url: string;
...
const VenueGallery: React.FC<VenueGalleryProps> = ({
  ...
  const currentMainImageAlt = images.find(img => img.url === currentMainImageUrl)?.alt || `Main view of ${venueName}`;

  return (
    <div className="space-y-3"> {/* Use space-y-3 for consistency */}
      {/* Main Image Display */}
      <div className="aspect-[16/9] overflow-hidden rounded-xl shadow-md bg-muted"> {/* Added shadow/bg */}
        <img
