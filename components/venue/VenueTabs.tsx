...
const VenueMap = dynamic(() => import("./VenueMap"), {
  ssr: false,
  loading: () => (
    <div className="aspect-[16/9] w-full rounded-lg bg-muted animate-pulse flex items-center justify-center border">
...
interface VenueTabsProps {
  ...
  reviews: SerializedPopulatedReview[]; // Use the serialized type
...
const VenueTabs: React.FC<VenueTabsProps> = ({
  ...
  return (
    <Tabs defaultValue="about" className="w-full">
      <TabsList className="grid w-full grid-cols-4 mb-6 bg-muted rounded-lg p-1 h-auto">
        ...
        {amenities && amenities.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {amenities.map((amenity) => {
              ...
              const IconComponent =
                amenityIconMap[iconKey] || DefaultAmenityIcon;
              return (
                // Use amenity._id or amenity.id for a stable key
                <div
                  key={amenity._id || amenity.id || amenity.label}
        ...
        {reviews && reviews.length > 0 ? (
          <>
            ...
            {totalReviewCount > DEFAULT_VISIBLE_REVIEWS && (
              ...
              >
                View {visibleReviews === totalReviewCount ? "Less" : "More"}{" "}
                Reviews
              </Button>
            )}
          </>
