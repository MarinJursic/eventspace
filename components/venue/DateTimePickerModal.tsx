...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setCurrentMonth(
        initialValidDates.length > 0 ? initialValidDates[0] : new Date()
      );
    }
  ...
  const handleDayPickerSelect: SelectMultipleEventHandler = (days) => {
  ...
  const isDateDisabled = (date: Date): boolean => {
    ...
    const isBooked = bookedDates.some((booked) =>
      isSameDay(new Date(booked.date), date)
    );
    ...
    if (availabilityRules?.blockedWeekdays) {
      ...
      const isBlocked = availabilityRules.blockedWeekdays.some(
        (rule) => rule.weekday.toLowerCase() === dayOfWeek.toLowerCase()
