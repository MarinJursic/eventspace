...
const formatDisplayPrice = (amount: number, model?: 'hour' | 'day' | 'week'): string => {
    ...
    switch (model) {
      case 'hour': return `${base}/hr`; // Shorter labels
      case 'day': return `${base}/day`;
      case 'week': return `${base}/wk`;
      default: return base; // Fallback if model is undefined or unexpected
    }
};
...
const VenueCard: React.FC<VenueCardProps> = ({ venue, className }) => {
  ...
  const imageUrl = venue.images?.[0]?.url || "https://via.placeholder.com/400x267?text=No+Image"; // Fallback image
  ...
  return (
    ...
    >
      ...
      {/* Image Section */}
      <div className="aspect-[16/10] w-full overflow-hidden bg-muted"> {/* Standard aspect ratio and bg */}
        <img
          src={imageUrl}
          alt={imageAlt}
      ...
      {/* Content Section */}
      <div className="flex flex-grow flex-col p-4"> {/* Standard padding */}
        {/* Name and Rating Row */}
        <div className="flex justify-between items-start gap-2 mb-1">
          <h3 className="font-semibold text-base leading-snug flex-1 mr-1"> {/* Standard text size */}
            {/* Link wrapping the name */}
            <Link href={`/venues/${venue._id}`} className="hover:underline focus:outline-none focus:ring-1 focus:ring-ring rounded-sm">
                {venue.name}
            </Link>
          </h3>
        ...
        {/* Price Display */}
        <div className="mt-2 text-sm"> {/* Consistent margin */}
        ...
        <div className="mt-4"> {/* Consistent margin */}
          ...
          >
            ...
            <Link href={`/venues/${venue._id}`} prefetch={false}>View Details</Link>
          </Button>
        </div>
      </div>
    </div>
