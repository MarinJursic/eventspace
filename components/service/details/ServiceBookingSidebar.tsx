...
import { MockService } from "@/lib/mocks/mockServices";
import { Button } from "@/components/ui/button";
import { Calendar, Check, Building, MapPin } from "lucide-react";
      ...
      <div className="bg-card rounded-xl p-6 shadow-lg border border-border">
        ...
        <div className="mb-4 pb-4 border-b">
          ...
          <p className="font-display text-3xl font-bold tracking-tight">
            ${servicePrice.basePrice.toLocaleString()}
          </p>
          {servicePrice.model === "day" && (
            <p className="text-xs text-muted-foreground mt-1">per day</p>
          )}
          {servicePrice.model === "hour" && (
            <p className="text-xs text-muted-foreground mt-1">per hour.</p>
          )}
          {servicePrice.model === "week" && (
        ...
        <div className="mb-6">
          ...
          {servicePrice.model === "day" &&
            ...
            selectedServiceDays.size > 0 && (
              <p className="text-xs text-muted-foreground mt-1 text-right">
                {" "}
                Based on ${servicePrice.basePrice.toLocaleString()}/day ×{" "}
                {selectedServiceDays.size} day(s) selected{" "}
              </p>
            )}
          ...
          {!(
            ...
            !(servicePrice.model === "hour") && (
              <p className="text-xs text-muted-foreground mt-1 text-right">
                {" "}
                Based on standard rate/package{" "}
              </p>
            )}
        </div>
          ...
          <div className="mb-6 space-y-4 pt-4 border-t">
            ...
            {availableDaysForSelection.length > 1 && (
              <div
                ...
                role="checkbox"
                aria-checked={availableDaysForSelection.every((day) =>
                  selectedServiceDays.has(day)
                )}
              >
                <div className="flex items-center">
                  <div
                    className={`w-5 h-5 rounded-full border-2 mr-3 flex items-center justify-center shrink-0 ${availableDaysForSelection.every((day) => selectedServiceDays.has(day)) ? "border-primary bg-primary" : "border-muted-foreground"}`}
                  >
                    {availableDaysForSelection.every((day) =>
                      selectedServiceDays.has(day)
                    ) && <Check className="w-3 h-3 text-primary-foreground" />}
                  </div>
                  <span className="text-sm font-medium">
                    All {availableDaysForSelection.length} Days
                  </span>
                </div>
              </div>
            )}
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded-md border cursor-pointer transition-all ${
                      !isAvailable
                        ? "opacity-50 cursor-not-allowed bg-muted/50 border-dashed" // Keep style for future complex rules
                        : isSelected
                  ...
                  >
                    <div className="flex items-center">
                      <div
                        className={`w-4 h-4 rounded-full border-2 mr-2 flex items-center justify-center shrink-0 ${!isAvailable ? "border-muted" : isSelected ? "border-primary bg-primary" : "border-muted-foreground"}`}
                      >
                        {isSelected && (
                      ...
                      <div className="flex items-center">
                        ...
                        <span
                          className={`text-sm ${!isAvailable ? "text-muted-foreground line-through" : ""}`}
                        >
                          {day}
                        </span>
                      </div>
        ...
        {hasVenue ? (
          ...
          >
            {isMultiDay && !isDaySelectionValid
              ? "Select Day(s) First"
              : "Add to Booking"}
          </Button>
        ) : (
        ...
        <p className="text-xs text-center text-muted-foreground mt-3">
          {" "}
          Finalize details in your booking overview.{" "}
        </p>
      </div>
