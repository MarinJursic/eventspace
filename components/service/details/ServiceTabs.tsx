    ...
    <TabsContent value="features">
        ...
        {features.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {features.map((feature) => {
                    // Get icon based on feature.key or feature.icon string
                    const IconComponent = serviceFeatureIconMap[feature.key] || DefaultFeatureIcon;
                    return (
                        // Use feature.id (virtual) or _id for key
                        <div key={feature.id || feature._id} className="flex items-start p-3 bg-secondary/30 rounded-lg border">
                            <IconComponent className="h-5 w-5 mr-3 text-primary shrink-0 mt-0.5" />
                            {/* Display the feature label */}
                            <span className="text-sm">{feature.label}</span>
                        </div>
...
=======
interface ServiceReviewsTabProps { reviews: SerializedReviewData[]; rating: SerializedRating; }
const ServiceReviewsTab: React.FC<ServiceReviewsTabProps> = ({ reviews = [], rating }) => {
     ...
     const totalReviewCount = rating?.count ?? reviews.length;
     const averageRating = rating?.average ?? 0;
     const handleToggleReviews = () => { setVisibleReviews(v => v === reviews.length ? DEFAULT_VISIBLE_REVIEWS : reviews.length); };
            ...
            {reviews.length > 0 ? (
                <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div><p className="font-medium">Based on {totalReviewCount} reviews</p></div>
                    </div>
                    {/* Review List */}
                    <div className="space-y-4">
                        {reviews.slice(0, visibleReviews).map((review) => (
                    ...
                    {/* View More/Less Button */}
                    {totalReviewCount > DEFAULT_VISIBLE_REVIEWS && (
                        <Button variant="outline" className="w-full mt-6" onClick={handleToggleReviews}>
...
interface ServicePoliciesTabProps { policies?: SerializedPolicies; }
