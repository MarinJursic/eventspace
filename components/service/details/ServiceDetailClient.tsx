...
interface ServiceDetailClientProps {
  service: SerializedPopulatedService | null;
}
...
const ServiceDetailClient: React.FC<ServiceDetailClientProps> = ({
  ...
  const handleAddToBooking = useCallback(() => {
    ...
    if (!initialService) {
      toast({
        title: "Error",
        description: "Service data missing.",
        variant: "destructive",
      });
      return;
    }
    if (!hasVenue) {
      toast({
        ...
        description: "Please select or add a venue first.",
        variant: "destructive",
      });
      return;
    }
    if (isMultiDay && !isDaySelectionValid) {
      toast({
        title: "Days Required",
        description: "Please select the day(s) this service is needed.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleExternalVenueAdd = useCallback(
    (venueName: string, location: string, dates: string[]) => {
  ...
  const toggleAllDays = useCallback(() => {
    const allVenueDaysSelected =
      selectedDates.length > 0 &&
      selectedDates.every((day) => selectedDays.has(day));
    if (allVenueDaysSelected) {
      setSelectedDays(new Set());
    } else {
      setSelectedDays(new Set(selectedDates));
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <AnimatedSection animation="fade-in">
                ...
                <ServiceTabs service={initialService} reviews={reviews} />
              </AnimatedSection>
            </div>
            {/* Right Column */}
